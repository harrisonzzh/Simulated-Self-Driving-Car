...
def random_rotation(image):
    ...
    rand_angle = 2*(np.random.rand()-0.5)

    M = cv2.getRotationMatrix2D((cols/2,rows/2),rand_angle,1)
    return cv2.warpAffine(img,M,(cols,rows))

def random_blur(image):
    kernel_size = 1+int(np.random.rand()*2)
    if(kernel_size%2 == 0):
...
def batch_generator(data_dir, image_paths, steering_angles, batch_size, is_training):
    ...
    while True:
        ...
        for index in np.random.permutation(image_paths.shape[0]):
            ...
            # argumentation
            # 60% chance image will be added some random noise (brightness, shadow, translation...)
            ...
            if i == batch_size:
                break
        return images.astype(np.float32), steers
